...
(function() {
  window.init_map = function() {
    var input, map_options, searchBox;
    map_options = {
      zoom: 10,
    ...
    if (input !== null) {
      ...
      google.maps.event.addListener(searchBox, 'places_changed', function() {
        var bounds, place, places, _i, _len;
    ...
    if (input !== null) {
      return map.controls[google.maps.ControlPosition.TOP_LEFT].push(input);
    }
  };

  window.init_map_place_selection = function() {
    init_map();
    return google.maps.event.addListener(window.map, 'click', function(e) {
      var foo;
      if (window.selection_marker != null) {
      ...
      return foo.value = e.latLng.lng();
    });
  };

  window.load_parties_in_zone = function() {
    ...
    return $.ajax({
      url: "/parties/get_parties_in_zone?zone=" + bounds.toUrlValue()
    }).done(function(json) {
      var content, coords, div, parties, party, _j, _len1, _results;
      parties = json;
      ...
      _results = [];
      for (_j = 0, _len1 = parties.length; _j < _len1; _j++) {
        party = parties[_j];
        content = "<div> <div></div> <div> <a href='/parties/" + party.id + "'><h3>" + party.title + "</h3></a> <p>" + party.summary + "</p> <span>" + party.date + "</span> </div> </div>";
        div.innerHTML += content;
        coords = new google.maps.LatLng(party.coord_latitude, party.coord_longitude);
        ...
        _results.push(google.maps.event.addListener(marker, 'click', function() {
          ...
          return infoWindow.open(window.map, this);
        }));
      }
      return _results;
    });
